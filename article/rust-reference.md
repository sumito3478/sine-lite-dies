...
この文書は [Rust リファレンス](http://doc.rust-lang.org/reference.html)の和訳です。

- この翻訳は試訳です。*既知の*誤訳、良くない訳、一貫性のない訳がまだ修正されていない部分があります。
- この翻訳は部分訳です。全体の一部しか翻訳されていません。
- 原文は[5月15日時点のの stable ブランチのソースコード][english_source]を参照しています。

[english_source]: https://github.com/rust-lang/rust/blob/a59de37e99060162a2674e3ff45409ac73595c0e/src/doc/reference.md

# はじめに

この文書はプログラミング言語Rustの一級資料である。以下の 3 種類の章で構成されている。

  - 言語の構成要素とその使用法について非形式的を説明する章。
  - メモリモデル、並行モデル、ランタイム機能、リンクモデルとデバッグ機能について非形式的に説明する章。
  - 理論的根拠と Rust のデザインに影響を与えた言語への参照を与える付録の章。

この文書は Rust の入門書ではない。Rust に慣れ親しんでいることが前提になる。
Rust に慣れるのを手助けするための[本](book)は別途ある。

この文書には Rust に含まれる[標準ライブラリ](standard)のリファレンスも含まれていない。
標準ライブラリはソースコードからドキュメンテーション属性を抽出することで別途文書化されている。
多くの人が言語機能だと期待する機能が Rust ではライブラリとして実現されていることがある。
そのため、読者が調べていることはこのリファレンスにではなく、標準ライブラリのリファレンスにある可能性がある。

また、[文法](grammar)のリファレンスもある。
...
Rust の文法を構成する生成規則のいくつかは、ASCII の範囲外の Unicode コードポイントを含むことを許している。
こうした生成規則は、ASCII 範囲のコードポイントで定義する代わりに、
Unicode 標準で定められた文字プロパティで定義している。
文法の [Unicode で定義された生成規則][unicodeproductions] ではこうした生成規則について説明している。

[unicodeprodunctions]: grammar.html#special-unicode-productions

## 文字列テーブルの生成規則

いくつかの規則 &mdash; 特に[単項演算子](#unary-operator-expressions)、
[二項演算子](#binary-operator-expressions)、[キーワード][keywords] &mdash; は
単純な形式で与えている。すなわち、引用符で囲んでいない、印字可能な、スペースで区切られた
文字列のテーブルのリストである。
These cases form a subset of the rules regarding
...
二重引用符（`"`）で囲まれている文字列が文法定義に現れた場合、
文字列表規則のメンバへの参照である。詳しくは[tokens](#tokens)を参照。

# 字句構造

## 入力形式

Rust の入力は UTF-8 でエンコードされた Unicode コードポイントの列として解釈される。
Rust の文法規則の多くは印字可能な ASCII 範囲のコードポイントとして定義されている。
しかしいくつかは Uniocode プロパティかコードポイントのリストで定義されている。[^inputformat]

[^inputformat]: この文書に記述されている文法を検証するときには、文法の検証器には、
  Unicode 生成規則には代わりの定義が与えられる。
  かわりの定義は ASCII 範囲に限定されている。

## 識別子

識別子は空文字列でない Unicode 文字列[^non_ascii_idents]で、
[キーワード][keywords]に含まれ_ない_もののうち、
以下の形式をとるものである。

[^non_ascii_idents]: 識別子中の ASCII 範囲外の文字は現状 feature gated である。
  まもなく改善される見込みである。

- 最初の文字がプロパティ `XID_start` を持つ。
- 残りの文字がプロパティ `XID_continue` を持つ。

> **メモ**: `XID_start` と `XID_continue` は一般的な C 系・Java 系言語の識別子を構成する
> 文字を表す文字プロパティである。

## コメント

RustのコメントはC++スタイルの行形式（`//`）・ブロック形式（`/* ... */`）のコメントである。
ネストしたブロックコメントもサポートされている。

ちょうど _3_ つのスラッシュ（`///`）で始まる行コメントと、
ブロックの開始にちょうど一つの連続するアスタリスクがある（`/**`）ブロックコメントは、
`doc`[属性](#attributes)のための特殊な構文とみなされる。
すなわち、コメントの周辺に`#[doc="..."]`と書くのとおなじである。
`/// Foo`は`#[doc="Foo"]`になる。

`//!`で始まる行コメントと`/*!`で始まるブロックコメントは
コメントの下にあるものではなく、コメントを含んでいる親に適用されるdocコメントである。
すなわち、`#![doc="..."]` と書くのと同等である。
`//!` コメントは通常モジュールのドキュメンテーションを書くのに使われる。

docコメントでないコメントは空白として解釈される。

## 空白

空白は以下の文字のみを含む空でない文字列である。
...
Rustは「フリーフォーム」の言語であり、Rustの文法において空白はトークンを
分割するためだけに使われる。意味論に影響はない。

空白部分を他のどんな空白、たとえばスペース一つに置き換えても、Rustのプログラムは
同一のままである。

## トークン

トークンはRustの文法のプリミティブな生成規則で、正規（非再帰）言語で定義される。
「単純な」トークンは、[文字列テーブルの生成規則](#string-table-productions)の形式で
与えられ、文法のそれ以外の場所では二重引用符で囲まれた文字列として現れる。
それ以外のトークンは記述されたとおりの規則を持つ。

### リテラル

リテラルは複数のトークンの列ではなく、一つのトークンで構成される式で、
即座に直接に、そのリテラルが評価された値を表す。
名前やその他の評価ルールで参照された値ではない。
リテラルは定数式を表す形式であり、コンパイル時に評価される。

#### 例

##### 文字と文字列

|                                              | Example         | `#` sets   | Character  s  | Escapes             |
|----------------------------------------------|-----------------|------------|---------------|---------------------|
| [文字](#character-literals)                  | `'H'`           | `N/A`      | Unicodeすべて | `\'` & [Byte](#byte-escapes) & [Unicode](#unicode-escapes) |
| [文字列](#string-literals)                   | `"hello"`       | `N/A`      | Unicodeすべて | `\"` & [Byte](#byte-escapes) & [Unicode](#unicode-escapes) |
| [Raw文字列](#raw-string-literals)            | `r#"hello"#`    | `0...`     | Unicodeすべて | `N/A`                                                      |
| [Byte](#byte-literals)                       | `b'H'`          | `N/A`      | ASCIIすべて   | `\'` & [Byte](#byte-escapes)                               |
| [Byte文字列](#byte-string-literals)          | `b"hello"`      | `N/A`      | ASCIIすべて   | `\"` & [Byte](#byte-escapes)                               |
| [Rawバイト文字列](#raw-byte-string-literals) | `br#"hello"#`   | `0...`     | ASCIIすべて   | `N/A`                                                      |

##### Byteエスケープ

|   | 名前 |
|---|------|
| `\x7F` | 8ビット文字コード（ちょうど数字2文字）) |
| `\n` | 改行 |
| `\r` | キャレッジリターン |
| `\t` | タブ |
| `\\` | バックスラッシュ |
...
|   | Name |
|---|------|
| `\u{7FFF}` | 24-bitのUnicode文字コード（数字6文字まで） |

##### 数値

| [数値リテラル](#number-literals)`*` | 例 | 指数部 | サフィックス |
|----------------------------------------|---------|----------------|----------|
| 十進整数 | `98_222` | `N/A` | 整数接尾辞 |
| 十六進整数 | `0xff` | `N/A` | 整数接尾辞 |
| 八進整数 | `0o77` | `N/A` | 整数接尾辞 |
| 二進整数 | `0b1111_0000` | `N/A` | 整数接尾辞 |
| 浮動小数点数 | `123.0E+77` | `Optional` | 浮動小数点数接尾辞 |
...
_文字リテラル_は二つの`U+0027`（単一引用符）に囲まれた一個の任意のUnicode文字である。
ただし`U+0027`自体は例外で、先行する`U+005C`（`\`）でエスケープしなければならない。

##### 文字列リテラル

_文字列リテラル_は二つの`U+0022`（二重引用符）で囲まれた任意のUnicode文字の列である。
ただし`U+0022`自体は例外で、先行する`U+005C`（`\`）でエスケープしなければならない。

文字列リテラルでは改行文字が許される。
通常は改行文字はその改行文字自体を表す（すなわち、変換はされない）。
しかし例外として、改行の直前に`U+005C`（`\`）が現れた場合、
その改行と、次の行の最初に現れた全ての空白が無視される。
したがって`a`と`b`は同等である。
...
いくつかの追加の_エスケープ_が文字リテラルやRawでない文字列リテラルで使える。
エスケープは`U+005C`（`\`）ではじまり、以下の形式のうちの一つが続く。

* _8ビットコードポイントのエスケープ_は`U+0078`（`x`）ではじまり、
  二つの_十六進数字_が続く。十六進数字で与えられた値ののUnicodeコードポイントを表す。
* _24ビットコードポイントエスケープ_は`U+0075`（`u`）ではじまり、
  `U+007B`（`{`）と`U+007D`（`}`）で囲まれた6文字までの_十六進数字_が続く。
  十六進数で与えられた値のUnicodeコードポイントを表す。
*  _空白文字のエスケープ_は`U+006E` (`n`)、`U+0072`
  (`r`)、`U+0074` (`t`)のうちの一つで、Unicode文字の`U+000A` (LF),
  `U+000D` (CR) or `U+0009` (HT)を表す。
* _バックスラッシュのエスケープ_は`U+005C` (`\`)で、バックスラッシュ_自身_を表すために
  エスケープされなければならない。


##### Raw文字列リテラル

Raw文字列リテラルはエスケープを処理しない。
`U+0072` (`r`)ではじまり、0個以上の`U+0023` (`#`)が続き、
`U+0022` （二重引用符）が続く。_Raw文字列の中身_はあらゆるUnicode文字の列を含むことができ、
もうひとつの`U+0022` （二重引用符）と、その後の、
最初の`U+0022` （二重引用符）に続いた
`U+0023` (`#`)と同じ個数の
`U+0023` (`#`)のみによって終わる。

Raw文字列に含まれるUnicode文字は全て自分自身を表す。
`U+0022` （二重引用符）（Raw文字列リテラルを開始するのに使われた`U+005C` (`\`)と
同じ個数の`U+005C` (`\`)が続く二重引用符を除く）や
`U+005C` (`\`)は特別な意味を持たない。
...
_Byteリテラル_は単一のASCII文字（`U+0000` から `U+007F` までの文字
または単一の _エスケープ_に、
`U+0062` (`b`) と `U+0027` （単一引用符）が先行し、`U+0027`が続いたものである。
`U+0027` がByteリテラル中で表される場合、
先行する`U+005C` (`\`)で_エスケープ_されなければならない。
Byteリテラルは`u8`符号なし8ビット整数_数値リテラル_と同等である。
...
Rawでない_Byte文字列リテラル_は、ASCII文字と、
`U+0062` (`b`) と `U+0022` （二重引用符）に先行され、
`U+0022`が続くASCII文字と_エスケープ_の列である。

リテラル中で`U+0022` （二重引用符）が現れる場合、先行する`U+005C` (`\`)で_エスケープ_され
なければならない。
エスケープする代わりに、Byte文字列リテラルは_Rawバイト文字列リテラル_になることができる。
Rawバイト文字列は下で定義される。
長さが`n`のバイト文字列リテラルは符号なし8ビット整数の固定長配列`&'static [u8; n]`と同等である。
...
いくつかの追加の_エスケープ_がバイトリテラルやRawでないバイト文字列リテラルで使える。
エスケープは`U+005C`（`\`）ではじまり、以下の形式のうちの一つが続く。

* _バイトエスケープ_は`U+0078`（`x`）ではじまり、
  ちょうど二つの_十六進数字_が続く。十六進数字で与えられたバイトを表す。
*  _空白文字のエスケープ_は`U+006E` (`n`)、`U+0072`
  (`r`)、`U+0074` (`t`)のうちの一つで、Unicode文字の`U+000A` (LF),
  `U+000D` (CR) or `U+0009` (HT)を表す。
* _バックスラッシュのエスケープ_は`U+005C` (`\`)で、バックスラッシュ_自身_を表すために
...
Rawバイト文字列リテラルはいかなるエスケープも行わない。
Rawバイト文字列は `U+0062` (`b`) からはじまり、 followed by `U+0072` (`r`) が続き、
0個以上の `U+0023` (`#`) と 1 個の `U+0022` （二重引用符）が続く。
_Raw 文字列の中身_ はあらゆる ASCII 文字列を含むことができ、
もうひとつの `U+0022` （二重引用符）に、
最初の `U+0022` （二重引用符）に続いていたのと同じ数の `U+0023` (`#`) が続くことによってのみ終わる。
Raw バイト文字列は ASCII 外のバイトを含むことができない。

Raw バイト文字列に含まれるUnicode文字は全て対応する ASCII エンコーディングを表す。
`U+0022`や`U+005C` (`\`) は特別な意味を持たない
（ただし、`U+0022`については、最初の `U+0022` （二重引用符）に続いていたのと
少なくとも同じ数の `U+0023` (`#`) が続く場合を除く）。
...
_数値リテラル_は_整数リテラル_または_浮動小数点数リテラル_である。
2種類のリテラルを認識する文法が混ざっている。

##### 整数リテラル

An _integer literal_ has one of four forms:
_整数リテラル_は以下の 4 種類の形式のうちの 1 つをとる。

*  _十進リテラル_は*十進数字*から始まり、*十進数字*と_アンダースコア_の列が続く。
*  _十六進数リテラル_は文字列 `U+0030` `U+0078`
  (`0x`) から始まり、十六進数字とアンダースコアの列が続く。
* `U+0030` `U+006F`
  _八進数リテラル_は文字列 `U+0030` `U+006F`
  (`0o`) から始まり、八進数字とアンダースコアの列が続く。
* _二進数リテラル_は文字列 `U+0030` `U+0062`
  (`0b`) から始まり、二進数字とアンダースコアの列が続く。

他のリテラルと同様に、整数リテラルの後（スペース無しに直後）には、
_整数サフィックス_が続くことができる。整数サフィックスはリテラルの
型を強制する。整数サフィックスは整数型のうちの一つと同じ名前でなければならない。
すなわち、`u8`, `i8`, `u16`, `i16`, `u32`, `i32`, `u64`, `i64`,
`isize`, `usize` のどれかである。

_サフィックスのない_整数リテラルの型は型推論によって決定される。
整数型が周囲のプログラムの文脈によって_一意に_決定される場合、
サフィックスのない整数リテラルはその型を持つ。
プログラムの文脈による型の制約が足りない場合、符号付き 32 ビット整数の `i32` になる。
プログラムの文脈による型の制約が多すぎる場合、静的型エラーとみなされる。
...
0xff_u8;                           // u8 型
0o70_i16;                          // i16 型
0b1111_1111_1001_0000_i32;         // i32 型
0usize;                            // usize 型
```
...
_浮動小数点数リテラル_は以下の 2 つの形式のうちの 1 つをとる。

* A _decimal literal_ followed by a period character `U+002E` (`.`). This is
  optionally followed by another decimal literal, with an optional _exponent_.
* _十進数リテラル_にピリオド文字 `U+002E` (`.`) が続く。
  その後にオプショナルなもうひとつの十進数リテラルと、
  オプショナルな_指数部_が続く。
* ひとつの_十進数リテラル_に_指数部_が続く。

整数リテラルと同様に、浮動小数点数リテラルにサフィックスを続けることができる。
ただし、サフィックスの前の部分が `U+002E` (`.`) で終わらない場合に限る。
サフィックスはリテラルの型を強制する。
_浮動小数点数接尾辞_には `f32` と `f64` があり（32-bit と 64-bit 浮動小数点数型）、
明示的にリテラルの型を決定する。

_サフィックスのない_浮動小数点数リテラルの型は型推論によって決定される。
浮動小数点数型が周囲のプログラムの文脈によって_一意に_決定される場合、
サフィックスのない浮動小数点数リテラルはその型を持つ。
プログラムの文脈による型の制約が足りない場合、倍精度の `f64` になる。
プログラムの文脈による型の制約が多すぎる場合、静的型エラーとみなされる。

さまざまな形式による浮動小数点数リテラルの例：
...
ピリオドで終わる浮動小数点数には接尾辞構文が使えないので、最後の例では接尾辞を使っていない。
`2.f64` というコードは　`2` のメソッド `f64` を呼ぼうとしてしまう。

浮動小数点数の意味論は ["Machine Types"](#machine-types) で説明される。


#### 真偽値リテラル

真偽値型の二つの値は `true` と `false` で書かれる。


### シンボル

シンボルは印字可能な [トークン](#tokens) の一般的なクラスで、
さまざまな文法規則において構造的役割を果たす。
すなわち、[単行演算子](#unary-operator-expressions)や[二項演算子](#binary-operator-expressions)や
[キーワード][keywords]に現れない残りの、印字可能な様々なトークンである。
完全のためにここで挙げた。


## パス

_パス_は名前空間修飾子 (`::`) によって_論理的に_区切られた 1 つ以上のパスのコンポーネントの列である。
パスがコンポーネント 1 つだけによって構成される場合、
ローカルの制御スコープにある [アイテム](#items) か [変数](#variable) のどちらかを参照する。
パスが複数のコンポーネントを持つなら、アイテムを参照する。

全てのアイテムはクレートの中で_正規化されたパス_を持つ。
しかしアイテムを名前付けるパスはクレートの中でのみ意味を持つ。
アイテムの正規化されたパスはクレートの中でのみアイテムを識別するだけである。

識別子コンポーネントだけで構成された単純なパスの例 2 つ：
...
パスコンポーネントは通常[識別子](#identifiers)であるが、
角括弧で囲まれた型引数のリストも含みうる。
[式](#expressions)のコンテキスト内では、型引数リストは `::` 名前空間識別子の後に与えられる。
より小さいの記号 (`<`) を含む比較式から区別するためである。
型式の文脈では、最後の名前空間識別子は省略される。
...
# fn f() {
# fn id<T>(t: T) -> T { t }
type T = HashMap<i32,String>; // 型式の中で使われる型引数
let x  = id::<i32>(10);       // 呼び出し式の中で使われる型引数
# }
```

パスはさまざまな先行する修飾子によって表示され、それによってパス解決の方法が変わる。

* `::` で始まるパスはグローバルパスとみなされ、
  パスのコンポーネントはクレートのルートから解決が始まる。
  パスのに含まれる識別子はそれぞれアイテムに解決されなければならない。
...
* キーワード `super` で始まるパスは親モジュールの相対パスから解決が始まる。
  以降の識別子はそれぞれアイテムに解決されなければならない。
...
* キーワード `self` で始まるパスは現在のモジュールの相対パスから解決が始まる。
  以降の識別子はそれぞれアイテムに解決されなければならない。
